...
class media_plugin(Plugin):
    ...
    def send_force_image(event):
        ...
        # 参数：图片URL或二进制数据, 文本内容
        event.reply_image(image_url, "这是使用reply_image方法发送的普通图片")

    @staticmethod
    def send_voice(event):
        """发送语音示例"""
        voice_url = "https://i.elaina.vin/api/tts/audio/audio_301007199ba99e254fc011a05262f030.mp3"
        
        # 参数：语音文件URL或二进制数据，自动转换为silk格式
    ...
    def send_markdown_template(event):
        ...
        image_url = "https://gchat.qpic.cn/qmeetpic/0/0-0-52C851D5FB926BC645528EB4AB462B3D/0"
        size_info = event.get_image_size(image_url)
        px_value = size_info['px']
        # 新的markdown模板方法 请查看core/event/markdown_templates.py，或者直接使用模板id
    ...
    def send_ark23(event):
        ...
        list_items = [
            ['功能1: 图片发送'],                                            # 第1项：只有描述
            ['功能2: 语音发送'],                                            # 第2项：只有描述  
            ['功能3: 视频发送', 'https://i.elaina.vin/api/']                # 第3项：描述+链接 理论可以无限加数组
        ]
    ...
    def send_ark24(event):
        """发送ark24信息卡片示例"""
        # 参数：描述, 提示, 标题, 元描述, 图片, 链接, 子标题  自己发请换白名单链接，或不传
        event.reply_ark(24, (
