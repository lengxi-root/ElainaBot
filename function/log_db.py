...
class LogDatabasePool:
    ...
    def _create_connection(self):
        return self._safe_execute(self._create_connection_with_retry)
    
    def _check_connection(self, connection):
        return self._safe_execute(lambda: connection.ping(reconnect=True) or True) is not None
    
    def _close_connection_safely(self, connection):
        self._safe_execute(lambda: connection.close())
                ...
                for i in range(len(self._pool) - 1, -1, -1):
                    if i < len(self._pool):
                        ...
                        if current_time - conn_info['last_used'] > 300:
                            self._safe_execute(lambda: conn_info['connection'].close())
                            del self._pool[i]
...
class LogDatabaseManager:
    ...
    def _get_create_table_sql(self, table_name, log_type):
        ...
        if log_type == 'received':
            ...
            specific_fields = "`content` text NOT NULL,"
                ...
                for log in logs:
                    ...
                    if log_type == 'error':
                        ...
                        if 'raw_message' in log and log['raw_message']:
                            f.write(f"原始消息:\n{log['raw_message']}\n")
                    elif log_type in ['received', 'unmatched']:
