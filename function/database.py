...
class Database:
    ...
    def _add_user(self, user_id):
        """实际添加用户的数据库操作"""
        try:
            sql = "INSERT IGNORE INTO bot_users (user_id) VALUES (%s)"
    ...
    def _add_group(self, group_id):
        """实际添加群组的数据库操作"""
        try:
            sql = "INSERT IGNORE INTO bot_groups (group_id) VALUES (%s)"
    ...
    def _add_user_to_group(self, group_id, user_id):
        ...
        try:
            # 检查群组是否存在
            sql = "SELECT users FROM groups_users WHERE group_id = %s"
            result = DatabaseService.execute_query(sql, (group_id,))
            
            if not result:
                # 群组不存在，创建新记录
                users = [{"value": 1, "userid": user_id}]
                sql = "INSERT INTO groups_users (group_id, users) VALUES (%s, %s)"
                ...
                if not any(u.get("userid") == user_id for u in users):
                    ...
                    sql = "UPDATE groups_users SET users = %s WHERE group_id = %s"
    ...
    def get_user_count(self):
        """获取用户总数"""
        try:
            sql = "SELECT COUNT(*) AS count FROM bot_users"
    ...
    def get_group_count(self):
        """获取群组总数"""
        try:
            sql = "SELECT COUNT(*) AS count FROM bot_groups"
            result = DatabaseService.execute_query(sql)
            return result.get('count', 0) if result else 0
        except Exception as e:
