...
def get_ssl_status():
    ...
    try:
        ...
        return jsonify({
            ...
            'domain': SERVER_CONFIG.get('ssl_domain', ''),
            'email': SERVER_CONFIG.get('ssl_email', ''),
            'cert_exists': False,
            'expiry': None,
            'remaining_days': -1,
            'ssl_port': SERVER_CONFIG.get('ssl_port', 8443),
            'check_hour': SERVER_CONFIG.get('ssl_check_hour', 12),
            'renew_days': SERVER_CONFIG.get('ssl_renew_days', 2),
            'cert_updated': False,
            'last_renew_time': None
        })
    except Exception as e:
    ...
    try:
        ...
        if not manager:
            if SERVER_CONFIG.get('ssl_auto_cert'):
                ...
                return jsonify({'success': False, 'error': 'SSL自动证书未启用，请在config.py中设置ssl_auto_cert为True'})
        ...
        def do_renew():
            ...
            if result:
                add_framework_log(f"SSL证书手动续签成功: {manager.domain}")
            else:
                add_error_log("SSL证书手动续签失败", "")
        
        threading.Thread(target=do_renew, daemon=True).start()
        return jsonify({'success': True, 'message': '证书续签已开始，成功后将自动重启'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
    ...
    try:
        ...
        if not manager:
            if SERVER_CONFIG.get('ssl_auto_cert'):
                ...
                return jsonify({'success': False, 'error': 'SSL自动证书未启用'})
        ...
        result = manager.prepare_precheck()
...
def ssl_renew_confirm():
    ...
    try:
        ...
        manager = get_ssl_manager()
        if not manager:
            return jsonify({'success': False, 'error': 'SSL管理器未初始化'})
        
        result = manager.confirm_renew()
        if result.get('success'):
            add_framework_log(f"SSL证书续签成功: {manager.domain}")
